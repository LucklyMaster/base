...
class EditableItemView @JvmOverloads constructor(
    ...
    private var leftText: EditableCellView? = null
    private var middleText: EditableCellView? = null
    private var rightText: EditableCellView? = null

    override val leftItem: EditableCellView
        get() {
            if (leftText == null) {
                leftText = EditableCellView(context)
            }
            return leftText!!
        }
    override val middleItem: EditableCellView
        get() {
            if (middleText == null) {
                middleText = EditableCellView(context)
            }
            return middleText!!
        }
    override val rightItem: EditableCellView
        get() {
            if (rightText == null) {
                rightText = EditableCellView(context)
            }
            return rightText!!
        }
