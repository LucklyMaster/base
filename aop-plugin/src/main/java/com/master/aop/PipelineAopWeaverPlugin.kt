...
class PipelineAopWeaverPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        val extension = project.extensions.create(
            AopWeaveExtension.AOP_WEAVE_EXTENSION, AopWeaveExtension::class.java
        )
        ...
        project.afterEvaluate {
            ...
            variants.forEach { variant ->
                ...
                val aopWeaveProvider = project.tasks
                    .register(
                        AOP_WEAVE_TASK.format(variantNameCapitalized), AopWeaveTask::class.java
                    )
                ...
                if (kaptTaskProvider != null) {
                    configureKaptTask(
                        kaptTaskProvider, kotlinCompileProvider, javaCompileProvider,
                        preWeaveJavaDir, preWeaveKotlinDir
                    )
                }
                ...
                if (javaCompileProvider != null) {
                    configureJavaCompileTask(
                        project, javaCompileProvider, preWeaveKotlinDir, preWeaveJavaDir
                    )
                }
                ...
                if (extractAnnotationsTaskProvider != null) {
                    configureExtractAnnotationsTask(
                        extractAnnotationsTaskProvider, preWeaveKotlinDir
                    )
                }
                ...
                variant.registerPostJavacGeneratedBytecode(
                    project.files(aopWeaveProvider.map { it.outputDir!! })
                )
            }
        }
    }
    ...
    private fun configureAopWeaveTask(
        ...
        aopWeaveProvider.configure {
            ...
            aspectPathResolver = {
                project.files(
                    gatherAspectPaths(
                        project, variantNameCapitalized, preWeaveJavaDir, preWeaveKotlinDir
                    )
                )
                    .also {
                        if (extension.filter.isNotEmpty()) {
                            project.aopLog(
                                "Weaving filter in ${project.name} is: ${extension.filter}"
                            )
                        }
                    }
                    .filter { file ->
                        println("path = ${file.canonicalPath}")
                        if (file.canonicalPath.contains("R.jar")) {
                            return@filter false
                        }
                        if (file.canonicalPath.contains("aop\\build\\intermediates\\compile_library_classes_jar\\debug\\classes.jar")){
                            return@filter true
                        }
            ...
            val dependencies = when {
                ...
                (kotlinCompileProvider != null) -> arrayOf(kotlinCompileProvider)
                    ...
                    "Either Java, Kotlin, or both types of compile task must be present."
                )
            }
    ...
    private fun restoreIncrementalBehaviorAction(
        compileTask: Task,
    ...
    private fun gatherAspectPaths(
        ...
        return when {
            ...
            (javaCompileTask != null) -> project.files(
                javaCompileTask.classpath, preWeaveJavaDir.get().asFile
            )
            (kotlinCompileTask != null) -> project.files(
                kotlinCompileTask.libraries, preWeaveKotlinDir.get().asFile
            )
            else -> throw RuntimeException(
                "Either Java, Kotlin, or both types of compile task must be present."
            )
    ...
    private fun Task.swapPreWeaveContent(
        compileTaskProvider: TaskProvider<Task>?,
        preWeaveDir: Provider<Directory>
    ) {
